...
class ArmoryMainWindow(QMainWindow):
   ...
   def __init__(self, parent=None, splashScreen=None):
      ...
      if USE_TESTNET:
         self.setWindowTitle('Armory - Unobtanium Wallet Management [TESTNET] dlgMain')
         ...
         self.setWindowTitle('Armory - Unobtanium Wallet Management [REGTEST] dlgMain')
         self.iconfile = ':/armory_icon_green_32x32.png'
         self.lblLogoIcon.setPixmap(QPixmap(':/armory_logo_green_h56.png'))
         if Colors.isDarkBkgd:
            self.lblLogoIcon.setPixmap(QPixmap(':/armory_logo_white_text_green_h56.png'))
      else:
         self.setWindowTitle('Armory - Unobtanium Wallet Management')
      ...
      # Setup system tray and register "unobtanium:" URLs with the OS
      ...
      self.lblBTC1 = QRichLabel('<b>UNO</b>', doWrap=False)
      self.lblBTC2 = QRichLabel('<b>UNO</b>', doWrap=False)
      self.lblBTC3 = QRichLabel('<b>UNO</b>', doWrap=False)
      ...
      btnSendBtc   = QPushButton(self.tr("Send Unobtaniums"))
      btnRecvBtc   = QPushButton(self.tr("Receive Unobtaniums"))
     ...
     # This is true if and only if the command line has a data dir that doesn't exist
      ...
      # This is true if and only if the command line has a unobtanium dir that doesn't exist
      #if not CLI_OPTIONS.satoshiHome in [BTC_HOME_DIR, DEFAULT]:
      #   QMessageBox.warning(self, self.tr('Unobtanium Directory'), self.tr(
   ...
   def setupSystemTray(self):
      ...
      actSendBtc    = self.createAction(self.tr('Send Unobtaniums'), traySend)
      actRcvBtc     = self.createAction(self.tr('Receive Unobtaniums'), trayRecv)
   ...
   def registerBitcoinWithFF(self):
      #the 3 nodes needed to add to register unobtanium as a protocol in FF
      rdfschemehandler = 'about=\"urn:scheme:handler:unobtanium\"'
      rdfscheme = 'about=\"urn:scheme:unobtanium\"'
      rdfexternalApp = 'about=\"urn:scheme:externalApplication:unobtanium\"'
      ...
      for rdfs in rdfs_found:
         if rdfs:
            ...
            if rdfsch == -1:
               FFrdf.write(' <RDF:Description RDF:about=\"urn:scheme:handler:unobtanium\"\n')
               FFrdf.write('                  NC:alwaysAsk=\"false\">\n')
               FFrdf.write('    <NC:externalApplication RDF:resource=\"urn:scheme:externalApplication:unobtanium\"/>\n')
               FFrdf.write('    <NC:possibleApplication RDF:resource=\"urn:handler:local:/usr/bin/xdg-open\"/>\n')
               FFrdf.write(' </RDF:Description>\n')
               i+=1

            if rdfsc == -1:
               FFrdf.write(' <RDF:Description RDF:about=\"urn:scheme:unobtanium\"\n')
               FFrdf.write('                  NC:value=\"unobtanium\">\n')
               FFrdf.write('    <NC:handlerProp RDF:resource=\"urn:scheme:handler:unobtanium\"/>\n')
               FFrdf.write(' </RDF:Description>\n')
               i+=1

            if rdfea == -1:
               FFrdf.write(' <RDF:Description RDF:about=\"urn:scheme:externalApplication:unobtanium\"\n')
   ...
   def setupUriRegistration(self, justDoIt=False):
      """
      Setup Armory as the default application for handling unobtanium: links
      """
      ...
      if OS_LINUX:
         out,err = execAndWait('gconftool-2 --get /desktop/gnome/url-handlers/unobtanium/command')
         out2,err = execAndWait('xdg-mime query default x-scheme-handler/unobtanium')
         ...
         def setAsDefault():
            ...
            execAndWait('gconftool-2 -t string -s /desktop/gnome/url-handlers/unobtanium/command "python2 %s \"%%s\""' % __file__)
            execAndWait('gconftool-2 -s /desktop/gnome/url-handlers/unobtanium/needs_terminal false -t bool')
            execAndWait('gconftool-2 -t bool -s /desktop/gnome/url-handlers/unobtanium/enabled true')
            execAndWait('xdg-mime default armory.desktop x-scheme-handler/unobtanium')
         ...
         rootKey = 'unobtanium\\shell\\open\\command'
         try:
         ...
         if action=='DoIt':
            ...
            regKeys = []
            regKeys.append(['Software\\Classes\\unobtanium', '', 'URL:unobtanium Protocol'])
            regKeys.append(['Software\\Classes\\unobtanium', 'URL Protocol', ""])
            regKeys.append(['Software\\Classes\\unobtanium\\shell', '', None])
            regKeys.append(['Software\\Classes\\unobtanium\\shell\\open', '',  None])
            ...
            regKeysU = []
            regKeysU.append(['Software\\Classes\\unobtanium\\shell\\open\\command',  '', \
                           modulepathname])
            regKeysU.append(['Software\\Classes\\unobtanium\\DefaultIcon', '',  \
   ...
   def startBitcoindIfNecessary(self):
      LOGINFO('startUnobtaniumdIfNecessary')

      TheSDM.checkDBIsLocal()

      if self.internetStatus == INTERNET_STATUS.Unavailable or \
         CLI_OPTIONS.offline:
         LOGWARN('Not online, will not start unobtaniumd')
         return False
      ...
      if TheSDM.satoshiIsAvailable():
         LOGWARN('Tried to start unobtaniumd, but satoshi already running')
         return False
      ...
      try:
         ...
         # path the executable.  That dir tree will be searched for unobtaniumd
         TheSDM.setupSDM(extraExeSearch=self.satoshiExeSearchPath)
         TheSDM.startBitcoind()
         LOGDEBUG('Unobtaniumd started without error')
         return True
      except:
   ...
   def setSatoshiPaths(self):
      ...
      if self.settings.hasSetting('SatoshiExe'):
         if not os.path.exists(self.settings.get('SatoshiExe')):
            LOGERROR('Unobtanium installation setting is a non-existent directory')
   ...
   def parseUriLink(self, uriStr, click=True):
      ...
      if TheBDM.getState() in (BDM_OFFLINE,BDM_UNINITIALIZED):
         LOGERROR('Clicked or entered "unobtanium:" link in offline mode.')
         self.bringArmoryToFront()
         if click:
            QMessageBox.warning(self, self.tr('Offline Mode'),
               self.tr('You clicked on a "unobtanium:" link, but Armory is in '
               'offline mode, and is not capable of creating transactions. '
               'Using links will only work if Armory is connected '
               'to the Unobtanium network!'), QMessageBox.Ok)
         else:
            QMessageBox.warning(self, self.tr('Offline Mode'),
               self.tr('You entered a "unobtanium:" link, but Armory is in '
               'offline mode, and is not capable of creating transactions. '
               'Using links will only work if Armory is connected '
               'to the Unobtanium network!'), QMessageBox.Ok)
         return {}
      ...
      if len(uriDict)==0:
         if click:
            warnMsg = (self.tr('It looks like you just clicked a "unobtanium:" link, but that link is malformed.'))
         else:
            warnMsg = (self.tr('It looks like you just entered a "unobtanium:" link, but that link is malformed.'))
         if self.usermode == USERMODE.Standard:
      ...
      if not uriDict.has_key('address'):
         if click:
            QMessageBox.warning(self, self.tr('The "unobtanium:" link you just clicked '
               'does not even contain an address!  There is nothing that '
               'Armory can do with this link!'), QMessageBox.Ok)
         else:
            QMessageBox.warning(self, self.tr('The "unobtanium:" link you just entered '
               'does not even contain an address!  There is nothing that '
               'Armory can do with this link!'), QMessageBox.Ok)
         LOGERROR('No address in "unobtanium:" link!  Nothing to do!')
         return {}
      ...
      if theAddrByte!=-1 and not theAddrByte in [ADDRBYTE, P2SHBYTE]:
         ...
         if click:
            QMessageBox.warning(self, self.tr('Wrong Network!'),
               self.tr('The address for the "unobtanium:" link you just clicked is '
            ...
            QMessageBox.warning(self, self.tr('Wrong Network!'),
               self.tr('The address for the "unobtanium:" link you just entered is '
               'for the wrong network!  You are on the <b>%2</b> '
               'and the address you supplied is for the '
               '<b>%3</b>!').arg(NETWORKS[ADDRBYTE], net), QMessageBox.Ok)
      ...
      for key,value in uriDict.iteritems():
         if key.startswith('req-') and not key[4:] in recognized:
            if click:
               QMessageBox.warning(self, self.tr('Unsupported URI'), self.tr('The "unobtanium:" link '
   ...
   def uriLinkClicked(self, uriStr):
      ...
      if TheBDM.getState()==BDM_OFFLINE:
         QMessageBox.warning(self, self.tr('Offline'),
            self.tr('You just clicked on a "unobtanium:" link, but Armory is offline '
   ...
   def finishLoadBlockchainGUI(self):
      ...
      if self.getSettingOrSetDefault('NotifyBlkFinish',True):
         reply,remember = MsgBoxWithDNAA(self, self, MSGBOX.Info,
            ...
            'receive unobtaniums.'), dnaaMsg=self.tr('Do not show me this notification again '), yesStr='OK')
   ...
   def createCombinedLedger(self, resetMainLedger=False):
      ...
      try:
         ...
         self.lblBTC1.setText('<b><font color="%s">UNO</font></b>' % lblcolor)
   ...
   def confirmSweepScan(self, pybtcaddrList, targAddr160):
      ...
      if TheBDM.getState() in (BDM_OFFLINE, BDM_UNINITIALIZED):
         ...
         msgConfirm = ( \
            ...
            'find any unobtaniums associated with the keys you supplied. '
         ...
         try:
            ...
            if errorMsgFromRPC == "success":
               QMessageBox.warning(self, self.tr('Transaction Broadcast'), self.tr(
                  ...
                  'of bad node connectivity to the Unobtanium network, or that your '
   ...
   def execImportWallet(self):
      ...
      bdm = TheBDM.getState()
      if sdm in ['UnobtaniumdInitializing', \
                 'UnobtaniumdSynchronizing'] or \
         bdm in [BDM_SCANNING]:
   ...
   def uriSendBitcoins(self, uriDict):
      ...
      descrStr = self.tr('You just clicked on a "unobtanium:" link requesting unobtaniums '
      ...
      if uri_has('amount'):
         ...
         descrStr += self.tr('<br>--<b>Amount</b>:\t%1 UNO').arg(amtstr)
   ...
   def clickReceiveCoins(self):
      ...
      if len(self.walletMap)==0:
         reply = QMessageBox.information(self, self.tr('No Wallets!'), self.tr(
            ...
            'nowhere to store your unobtaniums!  Would you like to '
            'create a wallet now?'), \
            QMessageBox.Yes | QMessageBox.No)
         if reply==QMessageBox.Yes:
            self.startWalletWizard()
   ...
   def setupDashboard(self):
      ...
      # to help the user install unobtaniumd.
      ...
      self.dashBtns[DASHBTNS.Close   ][BTN] = QPushButton(self.tr('Close Unobtanium Process'))
      self.dashBtns[DASHBTNS.Browse  ][BTN] = QPushButton(self.tr('Open http://unobtanium.uno'))
      ...
      #####
      def openBitcoinOrg():
         DlgBrowserWarn('http://unobtanium.uno').exec_()
      ...
      self.dashBtns[DASHBTNS.Close][LBL] = QRichLabel( \
           self.tr('Stop existing Unobtanium processes so that Armory can open its own'))
      self.dashBtns[DASHBTNS.Browse][LBL]     = QRichLabel( \
           self.tr('Open browser to Unobtanium webpage to download and install Unobtanium software'))
      self.dashBtns[DASHBTNS.Settings][LBL]  = QRichLabel( \
           self.tr('Open Armory settings window to change Unobtanium software management'))
      ...
      self.dashBtns[DASHBTNS.Browse][TTIP] = self.createToolTipWidget( self.tr(
           ...
           'and links about Unobtanium, in general.'))
      self.dashBtns[DASHBTNS.Settings][TTIP] = self.createToolTipWidget( self.tr(
           ...
           'a non-standard Unobtanium installation'))
   ...
   def closeExistingBitcoin(self):
      for proc in psutil.process_iter():
         try:
            if proc.name().lower() in ['unobtaniumd.exe','unobtanium-qt.exe',\
                                        'unobtaniumd','unobtanium-qt']:
   ...
   def GetDashFunctionalityText(self, func):
      ...
      if func.lower() == 'scanning':
         ...
         '<li>Sign messages</li>'
         '</ul>'
         '<br><br><b>NOTE:</b>  The Unobtanium network <u>will</u> process transactions '
         ...
         '<li>Send unobtaniums to other people</li>'
         '<li>Create one-time backups of your wallets (in printed or digital form)</li>'
         '<li>Click on "unobtanium:" links in your web browser '
   ...
   def GetDashStateText(self, mgmtMode, state):
      ...
      if state == 'NewUserInfo':
         ...
         'Here\'s some things you can do with Armory Unobtanium Client:'
         '<br>')
      elif state == 'OnlineFull2':
         return ( \
         (self.tr('If you experience any performance issues with Armory, '
         'please confirm that Unobtanium Core is running and <i>fully '
         'synchronized with the Unobtanium network</i>.  You will see '
         'a green checkmark in the bottom right corner of the '
         'Unobtanium Core window if it is synchronized.  If not, it is '
         ...
         'Unobtanium software running.  Most likely '
         ...
         '<br><br>'
         'If you do not have Unobtanium Core installed, you can '
         'download it from <a href="http://unobtanium.uno">'
         'http://unobtanium.uno</a>.')
      ...
      if mgmtMode.lower()=='user':
         if state == 'OfflineButOnlinePossible':
            ...
            'Unobtanium Core/unobtaniumd is fully synchronized with the unobtanium network.  '
            'You will see a green checkmark in the bottom-right corner of '
            'the Unobtanium Core window when it is finished.'
            '<br><br>'
            'Switching to online mode will give you access '
            'to more Armory functionality, including sending and receiving '
            'unobtaniums and viewing the balances and transaction histories '
            'of each of your wallets.<br><br>')
         elif state == 'OfflineNoSatoshi':
            bitconf = os.path.join(BTC_HOME_DIR, 'unobtanium.conf')
            return self.tr( \
            'You are currently in offline mode because '
            'Unobtanium Core is not running.  To switch to online '
            'mode, start Unobtanium Core and let it synchronize with the network '
            ...
            'Unobtanium Core in the background" under "File"->"Settings."'
            '<br><br>'
            'If you already know what you\'re doing and simply need '
            'to fetch the latest version of Unobtanium Core, you can download it from '
            '<a href="http://unobtanium.uno">http://unobtanium.uno</a>.')
            ...
            'that it is not necessary to have Unobtanium Core or unobtaniumd '
            'running.' )
         elif state == 'OfflineNoBlkFiles':
            return self.tr( \
            'You are currently in offline mode because '
            'Armory could not find the blockchain files produced '
            'by Unobtanium Core.  Do you run Unobtanium Core (or unobtaniumd) '
            ...
            return self.tr( \
            'Armory was previously online, but the connection to Unobtanium Core/'
            'unobtaniumd was interrupted.  You will not be able to send unobtaniums '
            'or confirm receipt of unobtaniums until the connection is '
            'reestablished.  <br><br>Please check that Unobtanium Core is open '
            'and synchronized with the network.  Armory will <i>try to '
            'reconnect</i> automatically when the connection is available '
            'again.  If Unobtanium Core is available again, and reconnection does '
         ...
         if state == 'OfflineUnobtaniumdRunning':
            return self.tr( \
            'It appears you are already running Unobtanium software '
            '(Unobtanium Core or unobtaniumd). '
            ...
            'Unobtanium application or adjust your settings.  If you change '
            'your settings, then please restart Armory.')
         if state == 'OfflineNeedUnobtaniumInst':
            return self.tr( \
            '<b>Only one more step to getting online with Armory!</b>   You '
            'must install the Unobtanium software from http://unobtanium.uno in order '
            'for Armory to communicate with the Unobtanium network.  If the '
            'Unobtanium software is already installed and/or you would prefer '
         ...
         if state == 'InitializingDoneSoon':
            ...
            'Unobtaniums forever!', "", len(self.walletMap))

            return msg
         if state == 'OnlineDisconnected':
            return self.tr( \
            'Armory\'s communication with the Unobtanium network was interrupted. '
            ...
            'all Unobtanium processes and restart Armory.').arg(os.path.basename(TheSDM.executable))
         if state == 'OfflineBadConnection':
            return self.tr( \
            'Armory has experienced an issue trying to communicate with the '
            'Unobtanium software.  The software is running in the background, '
            'but Armory cannot communicate with it through RPC as it expects '
            'to be able to.  If you changed any settings in the Unobtanium home '
         ...
         if state == 'OfflineSatoshiAvail':
            ...
            'running Unobtanium software.  Armory is in offline-mode. <br><br>'
            'If you are intending to run an offline system, you will not '
            'need to have the Unobtanium software installed on the offline '
         ...
         if state == 'OfflineForcedButSatoshiAvail':
            ...
            'running Unobtanium software.  If you are intending to run an '
            'offline system, you will <u>not</u> need to have the Unobtanium '
            'software installed or running on the offline '
            'computer.  It is only required for being online. ')
         if state == 'OfflineBadDBEnv':
            return self.tr( \
            'The Unobtanium software indicates there '
            'is a problem with its databases.  This can occur when '
            'Unobtanium Core/unobtaniumd is upgraded or downgraded, or sometimes '
            'just by chance after an unclean shutdown.'
            '<br><br>'
            'You can either revert your installed Unobtanium software to the '
            'last known working version (but not earlier than version 0.8.1) '
            'or delete everything <b>except</b> "wallet.dat" from your Unobtanium '
            'home directory '
            '<font face="courier"><b>%1</b></font>'
            '<br><br>'
            'If you choose to delete the contents of the Unobtanium home '
         ...
         if sdmStr == "NodeStatus_Offline":
            ...
            if self.internetStatus == INTERNET_STATUS.Unavailable or CLI_OPTIONS.offline:
               ...
               if TheSDM.satoshiIsAvailable():
                  ...
                  if CLI_OPTIONS.offline:
                     # Forced offline but unobtaniumd is running
            ...
            self.lblDashModeSync.setText(self.tr('Cannot find Unobtanium Home Directory'), \
                                                            size=4, bold=True)
            ...
            setBtnFrameVisible(True)
            descr1 += self.GetDashStateText('Auto', 'OfflineNeedUnobtaniumInst')
            ...
            if sdmStr == "NodeStatus_Initializing":
               self.lblDashModeSync.setText( self.tr('Initializing Unobtanium Engine'), size=4, bold=True, color='Foreground')
            ...
            setBtnRowVisible(DASHBTNS.Settings, True)
            setBtnFrameVisible(True, \
               self.tr('Since version 0.88, Armory runs unobtaniumd in the '
   ...
   def updateStatusBarText(self):
      if self.nodeStatus.status_ == Cpp.NodeStatus_Online:
         ...
         def getToolTipTextOffline():
            blkRecvAgo  = RightNow() - self.blkReceived
            tt = self.tr(
            'Disconnected from Unobtanium Node, cannot update history '
      ...
      if action == FINISH_LOAD_BLOCKCHAIN_ACTION:
         ...
         if prevStatus != self.nodeStatus.status_:
            ...
            if self.nodeStatus.status_ == Cpp.NodeStatus_Offline:
               self.showTrayMsg(self.tr('Disconnected'), self.tr('Connection to Unobtanium Core '
                                'client lost!  Armory cannot send nor '
                                'receive unobtaniums until connection is '
                                're-established.'), QSystemTrayIcon.Critical,
                                10000)
            elif self.nodeStatus.status_ == Cpp.NodeStatus_Online:
               self.showTrayMsg(self.tr('Connected'), self.tr('Connection to Unobtanium Core '
                                      're-established'), \
   ...
   def Heartbeat(self, nextBeatSec=1):
      ...
      try:
         ...
         if self.doAutoBitcoind:
            ...
            if (sdmState in ['UnobtaniumdInitializing','UnobtaniumdSynchronizing']) or \
               (sdmState == 'UnobtaniumdReady' and bdmState==BDM_SCANNING):
               self.updateSyncProgress()

         else:
            if bdmState in (BDM_OFFLINE,BDM_UNINITIALIZED):
               # This call seems out of place, but it's because if you are in offline
               # mode, it needs to check periodically for the existence of Unobtanium Core
   ...
   def printAlert(self, moneyID, ledgerAmt, txAmt):
      ...
      if ledgerAmt > 0:
         # Received!
         title = self.tr('Unobtaniums Received!')
         dispLines.append(self.tr('Amount:  %1 UNO').arg(totalStr ))
         dispLines.append(self.tr('Recipient:  %1').arg(dispName))
      elif ledgerAmt < 0:
         # Sent!
         title = self.tr('Unobtaniums Sent!')
         dispLines.append(self.tr('Amount:  %1 UNO').arg(totalStr))
   ...
   def doTheSystemTrayThing(self):
      ...
      for i in range(len(self.notifyQueue)):
         ...
         if le.isSentToSelf():
            ...
            # self.showTrayMsg('Your unobtaniums just did a lap!', \
            #             'Wallet "%s" (%s) just sent %s UNO to itself!' % \
            #         (wlt.labelName, moneyID, coin2str(amt,maxZeros=1).strip()),
            self.showTrayMsg(self.tr('Your unobtaniums just did a lap!'), \
                             self.tr('%1 just sent some UNO to itself!').arg(wltName), \
                             QSystemTrayIcon.Information, 10000)
            return
         ...
         if le.getValue() > 0:
            ...
            try:
               ...
               title = self.tr('Unobtaniums Sent!')
               dispLines.append(unicode(self.tr('Amount:  %1 UNO').arg(totalStr)))
   ...
   def unpackLinuxTarGz(self, targzFile, changeSettings=True):
      ...
      unpackDir  = os.path.join(ARMORY_HOME_DIR, 'latestUnobtaniumInst')
      unpackDir2 = os.path.join(ARMORY_HOME_DIR, 'latestUnobtaniumInstOld')
      ...
      LOGWARN('Unobtanium Core unpacked into: %s', finalDir)
